...
void Svgfile::addId(double x, double y, std::string text, std::string color)
{
    ...
    m_ostrm << "<text "
            ...
            << attrib("y", y)
            << attrib("fill", color)
            << attrib("text-anchor","middle")///pour centrer le texte
            << ">" << text << "</text>\n";
}
void Svgfile::addFleche(double x1, double y1, double x2, double y2, std::string color)
{
    m_ostrm << "<line "
            << attrib("x1", x1)
            << attrib("y1", y1)
